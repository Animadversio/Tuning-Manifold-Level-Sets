...
def get_Evol_Manif_stats(Animal, mat_path=mat_path):
    ...
    EStats = loadmat(join(mat_path, Animal + "_Evol_stats.mat"), struct_as_record=False,
...
def load_score_mat(EStats, MStats, Expi, ExpType, wdws=[(50, 200)], stimdrive="N"):
    ...
    ```
    """
    assert len(wdws) == 1, "Not implemented yet"
    wdwslice = slice(wdws[0][0], wdws[0][1])  # slice(50,200) by default
    if ExpType == "Evol":
        ...
        assert nunit == 1
        score_vect = np.mean(psthmat[0, wdwslice, :], axis=(0)).astype(np.float)
        ...
        else: # if multiple Manif is done, pick the first one PC23.
        ...
        scorecol = [np.mean(P[ui - 1, wdwslice, :], axis=0).astype(np.float) for P in psthlist]
        ...
        # ui=1; nunit = 1
        scorecol = [np.mean(P[wdwslice, :], axis=0).astype(np.float) for P in psth]
        ...
        if MStats[Expi - 1].ref.didGabor == 0:  # Gabor is not done
            ...
            psthlist = list(np.reshape(P, [nunit, 200, -1]) for P in psth)
            scorecol = [np.mean(P[ui - 1, wdwslice, :], axis=0).astype(np.float) for P in psthlist]  # avg over time
            idx_vect = [np.array(idx).reshape(-1) for idx in idx_vect]  # in case only 1 index
            imgnm_vect = [MStats[Expi - 1].imageName[idx[0] - 1] for idx in idx_vect]  # note offset the index bt
            # one, since it's matlab convention.
